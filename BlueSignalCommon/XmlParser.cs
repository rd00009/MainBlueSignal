...
using BlueSignalCommon.Common;
using System.Text;

namespace BlueSignalCommon
{
    using System.Data;
    ...
    public class XmlParser
    {
        ...
        public static string GetNodeValue(string xml)
        {
            ...
            var nodeValue = (from c in doc.Descendants("var")
                             where c.Attribute("name").Value == Common.Constants.Prize3OfflineCatIDs
        ...
        public static double GetNodeValuePrizeFactor(string xml)
        {
            var doc = XDocument.Parse(xml);
            var nodeValue = (from c in doc.Descendants("var")
                             where c.Attribute("name").Value == Common.Constants.PrizeFactor
                             select c.Value).SingleOrDefault();
        ...
        public static string GetPrizeOptionChoices(string xml, string attributeName)
        {
            ...
            if (attributeName == Common.Constants.AttributePrizeOption1)
                attributeName = Common.Constants.PrizeOption1;
            else if (attributeName == Common.Constants.AttributePrizeOptionChoices1)
                attributeName = Common.Constants.PrizeOption1Choices;
            else if (attributeName == Common.Constants.AttributePrizeOption2)
                attributeName = Common.Constants.PrizeOption2;
            else if (attributeName == Common.Constants.AttributePrizeOptionChoices2)
                attributeName = Common.Constants.PrizeOption2Choices;
